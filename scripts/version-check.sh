...
readonly LOCK_VERSION="$(awk '
  ...
  in_package && /^name = "alfred_vue_docs"$/ { name = "alfred_vue_docs"; next }
  in_package && name == "alfred_vue_docs" && /^version = / {
    gsub(/["[:space:]]/, "", $3); print $3; exit
  }
' Cargo.lock)"

if [[ -z "$MANIFEST_VERSION" || -z "$LOCK_VERSION" ]]; then
  echo "Could not read the alfred_vue_docs version from Cargo.toml and Cargo.lock" >&2
  exit 1
fi
