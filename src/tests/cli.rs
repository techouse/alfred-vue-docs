...
fn parse_reads_query_and_flags() -> Result<()> {
    ...
    assert_eq!(
        cli,
        Cli {
            query: "  v3   Request Body ".to_owned(),
            verbose: true,
            update: true,
        }
    );
...
fn normalized_query_removes_every_selected_version_token() {
    let cli = Cli {
        query: "  v3   Request Body v3 ".to_owned(),
        ..Cli::default()
    };

    assert_eq!(cli.normalized_query("v3"), "request body");
}

#[test]
fn normalized_query_removal_is_case_sensitive_before_lowercasing() {
    let cli = Cli {
        query: "V3 v3 composition".to_owned(),
        ..Cli::default()
    };

    assert_eq!(cli.normalized_query("v3"), "v3 composition");
}
