...
/// Configuration required to query the Vue.js Algolia index.
...
/// Synchronous client for the Vue.js documentation Algolia index.
#[derive(Clone, Debug)]
...
impl AlgoliaSearch {
    ...
    /// Searches the selected Vue.js version and returns ranked hits.
    ...
    fn request_host(&self, endpoint: &Url, body: &str, remaining: Duration) -> AttemptResult {
            ...
            let response: SearchResponse =
                serde_json::from_str(&response_body).map_err(|error| {
                    AttemptFailure::Terminal(anyhow!(
                        "failed to deserialize Algolia response from {endpoint} with HTTP status {status}: {error}; response body: {}",
                        bounded_body(&response_body)
                    ))
                })?;
        ...
        let error = anyhow!(
            "Algolia request to {endpoint} failed with HTTP status {status}: {}",
            bounded_body(&response_body)
        );
