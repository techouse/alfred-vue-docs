...
.PHONY: help build build-release build-release-binary build-arm64 build-x86_64 build-universal fmt fmt-check clippy test msrv licenses package version-check ci clean
...
help:
	...
	@printf '%-20s %s\n' 'msrv' 'Run all tests with Cargo.toml rust-version.'
...
build-release:
	...
	./scripts/package-workflow.sh target/release/alfred_vue_docs
...
build-universal: build-arm64 build-x86_64
	...
	lipo -create \
		target/aarch64-apple-darwin/release/alfred_vue_docs \
		target/x86_64-apple-darwin/release/alfred_vue_docs \
		-output target/universal-apple-darwin/release/workflow; \
...
msrv:
	@set -euo pipefail; \
	MSRV="$$(awk '/^\[package\]$$/ { in_package = 1; next } in_package && /^\[/ { exit } in_package && /^rust-version = / { gsub(/["[:space:]]/, "", $$3); print $$3; exit }' Cargo.toml)"; \
	test -n "$$MSRV"; \
...
package: build-universal
	...
	WORKFLOW_NAME="$${WORKFLOW_NAME:-vue-docs}"; \
	...
	(cd build/dist && zip -qr "../$${WORKFLOW_NAME}-v$${VERSION}.alfredworkflow.tmp.zip" . \
		-x '.env' 'query_cache/*' 'update_cache/*' '*_cache/*' '*_cache_keys.json' \
		   'workflow_intel' 'configure.png' 'demo.gif'); \
