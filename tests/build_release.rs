...
impl Fixture {
    fn new() -> Result<Self> {
        ...
        write_executable(
            &fake_bin.join("cargo"),
            "#!/usr/bin/env bash\nset -eu\nif [[ -n \"${UNRELATED_DOTENV+x}\" ]]; then exit 1; fi\nprintf '%s\\n' \"${ALGOLIA_APPLICATION_ID-}\" \"${ALGOLIA_SEARCH_ONLY_API_KEY-}\" \"${ALGOLIA_SEARCH_INDEX-}\" > \"$FAKE_CARGO_RECORD\"\n",
        )?;

        let scripts = root.join("scripts");
        fs::create_dir(&scripts)?;
        fs::copy(
            Path::new(env!("CARGO_MANIFEST_DIR")).join("scripts/build-release.sh"),
            scripts.join("build-release.sh"),
        )?;
        write_executable(
    ...
    fn command(&self) -> Command {
        ...
        command.env_remove("UNRELATED_DOTENV");
        command
    }
...
#[test]
fn build_release_exports_only_algolia_values_from_dotenv() -> Result<()> {
    let fixture = Fixture::new()?;
    fs::write(
        fixture.directory.path().join(".env"),
        "ALGOLIA_APPLICATION_ID=local-app\nALGOLIA_SEARCH_ONLY_API_KEY=local-key\nALGOLIA_SEARCH_INDEX=local-index\nUNRELATED_DOTENV=must-not-leak\n",
    )?;
